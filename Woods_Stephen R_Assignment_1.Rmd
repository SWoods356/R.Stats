---
title: 'Woods_Stephen R Assignment #1 (50 points)'
output:
...
par(bg = "aliceblue")
plot(x, y, main = "Y versus X", col = "blue",
     type = "l", pch = 19,  xlab = "x", ylab = "y",)
text(x=minx, y=miny, labels='(-1.571, -1.414)', pos = 3)
points(minx, miny, col = "red", pch = 19)
...
data.fun <- data.frame(x, y1, y2)


library(ggplot2)
library(ggthemes)

ggplot(data = data.fun, aes(x = x)) +
  ...
  geom_text(aes(x=0,y=0), color = "black", label = "Intersect (0,0)", nudge_y = .2)
...
apply(trees,2,mean)
tree.mean = mean(trees$Volume)

nrow(trees[trees$Volume > tree.mean, ])
...
trees[which.max(trees$area),][1:3]
...
x1 <- ggplot()+ aes(x) + geom_histogram(color = "grey", fill = "lightblue4", binwidth = .5)
x2 <- ggplot()+ aes(x) + geom_boxplot(color = "grey", fill = "lightblue4")
y1 <- ggplot()+ aes(y) + geom_histogram(color = "grey", fill = "coral2", binwidth = .5)
y2 <- ggplot()+ aes(y) + geom_boxplot(color = "grey", fill = "coral2")

ggarrange(x1, x2, y1, y2, 
          labels = c("X","X","Y","Y"),
          hjust = 0,
          ncol = 2, nrow = 2)
...
```{r test5c}
xqq <- ggplot() + aes(sample = x) + stat_qq(size = 2, col = "lightblue4") + stat_qq_line(col = "black") + ggtitle("X: Normal Distribution")
yqq <- ggplot() + aes(sample = y) + stat_qq(size = 2, col = "coral2") + stat_qq_line(col = "black") + ggtitle("Y: T Distribution")
...
print("There are no extreme outliers in x or y")
